    ...
    def __init__(self, dim_in=1, num_conv_block=2, kernel_root=8, use_bn=False):
        ...
        for n in range(num_conv_block):
            if n == 0:
                ...
                setattr(self, "conv%d" % (n + 1),
                        Conv3dBlock(kernel_root * (2 ** (n - 1)), kernel_root * (2 ** n), use_bn=use_bn))

        # UpConv Layers
        for n in range(num_conv_block - 1):
            i = num_conv_block - 1 - n
            setattr(self, "upconv%dto%d" % (i + 1, i),
...
class UNet2d(nn.Module):
    def __init__(self, dim_in=6, num_conv_block=3, kernel_root=4, use_bn=True):
        ...
        for n in range(num_conv_block):
            if n == 0:
                ...
                setattr(self, "conv%d" % (n + 1),
                        Conv2dBlock(kernel_root * (2 ** (n - 1)), kernel_root * (2 ** n), use_bn=use_bn))
    ...
    def forward(self, x):
        ...
        for n in range(num_conv_block - 1):
            # TODO: BUGGY, out may be uninitialized
            i = num_conv_block - 1 - n
            if n == 0:
                tmp = torch.cat(
                    (
...
class MultiSliceBcUNet(nn.Module):
    def __init__(self, num_slice=6, in_shape=256, num_conv_block=4, kernel_root=16, use_bn=True):
